...
def refresh():
    ...
    method=(ROOT/'output_q3_resource/模型建立.md').read_text(encoding='utf-8').split('## 上限情景与临界收益决策')[0]
    q3=method+'\n'+(ROOT/'output_q3_resource/模型求解.md').read_text(encoding='utf-8')
    q3=q3.replace('](figures/','](output_q3_resource/figures/')
    q4=(ROOT/'output_q4_evolution/模型建立.md').read_text(encoding='utf-8')+'\n'+(ROOT/'output_q4_evolution/模型求解.md').read_text(encoding='utf-8')
