...
# Z_j(u)&=\Phi^{-1}\bigl(\widetilde p_j(u)\bigr),\\
# \epsilon&=\frac{1}{2n}.
# \end{aligned}
# $$
# 
...
# &=\operatorname{clip}\bigl(p_i^{\mathrm{len}},\epsilon,1-\epsilon\bigr),\\
...
# &=\operatorname{clip}\bigl(p_{ij}^{\mathrm{domain}},\epsilon,1-\epsilon\bigr),\\
# Z_{ij}^{\mathrm{domain}}
# &=\Phi^{-1}\bigl(\widetilde p_{ij}^{\mathrm{domain}}\bigr).
# \end{aligned}
# $$
...
# $$
# CI>0 为冲突样本，E 为空则 CI=0。扩展集沿用 A1 指标对和高低阈值，不强制划出固定比例冲突。
...
# 
# **评价流程。** 使用训练集内嵌套交叉验证：外层 5 折（随机种子 0）产生每个样本的折外预测；有待选参数的模型只在外层训练部分再做内层 5 折（随机种子 1），按均方误差选参。线性基线没有调参。各模型最终参数另由完整 A4/A5 内部 5 折确定，再对完整训练集拟合。以外层折外 RMSE 最小者作为预测性能基准；RMSE 相同时按排序相关选择。主模型另按研究用途确定：综合预测能力、可解释性和约束配比求解便利性，采用对数线性。此项取舍不代表其预测误差最小，也不通过改变评价指标掩盖 GBM 的优势。参数选择与性能基准选择均不读取检验集或外推表结果。由于这些检验集在既往分析中已被查看，本次是纠正计算流程后的固定检验评价，不称为全新盲测。
# 
...
# $$
# 
# 该方向满足配比约束；g_i<0 表示在指定配比附近，该方向预测降低损失。对数线性作为解释和配比求解主模型，预测性能优胜模型由交叉验证单独确定，并用于辅助核查。
...
# 在读取任何检验预测结果前，按训练集的折外RMSE选择预测模型。
...
# 
# GBM 依据训练内折外 RMSE 成为预测性能基准；综合预测能力、解释性和配比求解便利性，对数线性作为主模型。其在 1B 检验排序上有优势，但这不是训练内选参依据。不能据训练集拟合优度或某一个检验尺度宣布跨尺度全面最优。完整 MAE、R²、Pearson 与分折参数见导出表。
...
# 
# 后续接口采用对数线性主模型的 KKT 解析配比，并用 SLSQP 数值解核验。GBM 作为性能基准，核查该配比与训练均值、均匀配比及其自身搜索候选的预测损失；模型间分歧如实报告，不将交叉评价称为真实训练验证。另保留对数线性与 GBM 的平均名次折中候选用于比较；它是模型折中启发式，不称为“稳健最优”。所有候选尚无真实训练验证，且跨尺度未校准，后续问题应把它们当作条件性方案。
...
# 
# 综合预测表现、解释性和配比求解便利性，采用“对数线性主建模与求解、GBM 性能基准与辅助核查”的分工，不把对数线性称为四模型预测冠军。领域组合影响使用总份额不变的有限转移，不以二阶系数正负推断互补机制。后续接口采用对数线性 KKT 解析配比，并经 SLSQP 核验；保留 GBM 搜索候选和两模型折中候选作为对照。GBM 对主方案的评价及分歧单独报告；均无真实训练验证。
# 
